...
type Row = { created_at: string; matiere: string | null; note_prof: number | null; comment_prof: string | null }
...
export default function Page(){
  ...
  async function generer(){
    ...
    const { data, error } = await supabase
      .from('proofs').select('*')
      .gte('created_at', `${monthKey}-01`).lte('created_at', `${monthKey}-31`)
    ...
    for(const n of notes){ const k = n.matiere || 'Général'; g[k]??={sum:0,count:0}; g[k].sum+=(n.note_prof||0); g[k].count++ }
    const pm = Object.entries(g).map(([m,{sum,count}])=>({ matiere:m, moyenne: Math.round(sum/count), count }))
    setParMatiere(pm)
    const total = pm.reduce((a,b)=>a+b.count,0) || 1
    setMoyenne(Math.round(pm.reduce((a,b)=>a+b.moyenne*b.count,0)/total))
    setLoading(false)
  }
  ...
  return (
    <main className="container">
      ...
      <ProfCard size="lg" subtitle="Je calcule tes moyennes par matière et prépare ton bulletin." />
      <div style={{height:12}}/>
          ...
          <>
            ...
            {parMatiere.length ? (
              <table className="table">
                ...
                <tbody>
                  {parMatiere.map((m)=>(
                    <tr key={m.matiere}><td>{m.matiere}</td><td>{m.count}</td><td>{m.moyenne}/20</td></tr>
                  ))}
                </tbody>
              </table>
            ...
            {rows.length ? (
              <table className="table">
                ...
                <tbody>
                  {rows.map((r,i)=>(
                    <tr key={i}><td>{r.created_at.slice(0,10)}</td><td>{r.matiere || 'Général'}</td><td>{r.note_prof}/20</td><td>{r.comment_prof || ''}</td></tr>
                  ))}
                </tbody>
              </table>
