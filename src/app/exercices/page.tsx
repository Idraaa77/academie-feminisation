...
const DAILY = [
  ...
  { id: 'makeup-1', title: 'Make-up: 1 technique', cat: 'Maquillage' },
        ...
        {DAILY.map(ex => (
          <label key={ex.id} style={{display:'flex',gap:8, alignItems:'center', padding:'8px 0', borderBottom:'1px solid #fde2e2'}}>
            ...
            <div>
              ...
              <div className="muted">Matière : {ex.cat}</div>
            </div>
            <div style={{marginLeft:'auto', display:'flex', gap:8}}>
              <input type="file" onChange={e=>setFile(e.target.files?.[0]||null)} />
              <button className="btn" onClick={()=>uploadProof(ex.id, ex.cat)}>Envoyer preuve</button>
            </div>
          </label>
        ))}
